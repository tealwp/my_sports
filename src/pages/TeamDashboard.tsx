...
export const TeamDashboard: React.FC<Props> = ({ adapter }) => {
  ...
  const renderGameLogTable = (games: IGameLog[]) => {
    ...
    const headers = Array.from(
      new Set(gameLog.flatMap((g) => g.stats.map((s) => s.label)))
    );
        ...
        <table className="w-full text-sm text-left border rounded-lg overflow-hidden">
          <thead className="bg-gray-50 text-gray-500 border-b">
            <tr>
              ...
              {headers.map((label) => (
                <th key={label} className="px-4 py-2 font-medium text-center">
                  {label}
                </th>
              ))}
            </tr>
            ...
            {games.map((game) => (
              ...
              >
                ...
                {headers.map((label) => {
                  ...
                  return (
                    <td key={label} className="px-4 py-2 font-mono text-center">
                      {stat?.value ?? "-"}
                    </td>
                  );
                })}
  ...
  const renderStatsSummary = (games: IGameLog[], title: string) => {
    ...
    if (isGoalie) {
      const wins = games.filter((g) =>
        g.stats.some((s) => s.label === "DEC" && s.value === "W")
      ).length;
      const losses = games.filter((g) =>
        g.stats.some((s) => s.label === "DEC" && s.value === "L")
      ).length;
      const goalsAgainst = games.reduce(
        (sum, game) =>
          sum + (Number(game.stats.find((s) => s.label === "GA")?.value) || 0),
        0
      );
      const shotsAgainst = games.reduce(
        (sum, game) =>
          sum + (Number(game.stats.find((s) => s.label === "SA")?.value) || 0),
        0
      );
      ...
      const goals = games.reduce(
        (sum, game) =>
          sum + (Number(game.stats.find((s) => s.label === "G")?.value) || 0),
        0
      );
      const assists = games.reduce(
        (sum, game) =>
          sum + (Number(game.stats.find((s) => s.label === "A")?.value) || 0),
        0
      );
      const points = goals + assists;
      const plusMinus = games.reduce(
        (sum, game) =>
          sum + (Number(game.stats.find((s) => s.label === "+/-")?.value) || 0),
        0
      );
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <Link
          to={`/${adapter.leagueName.toLowerCase()}`}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-gray-500 text-sm">
                    {player.position}{" "}
                    {player.number && <>&bull; #{player.number}</>}
                  </div>
                </div>
              </div>
            </div>
      ...
      {selectedPlayer && (
        <div
          className="fixed inset-0 z-50 flex justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto"
                      ...
                      <>
                        ...
                        {totalGameLogPages > 1 && (
                          <div className="flex justify-between items-center mt-4">
                            <button
                              onClick={() =>
                                setGameLogPage((p) => Math.max(1, p - 1))
                              }
                            ...
                            <button
                              onClick={() =>
                                setGameLogPage((p) =>
                                  Math.min(totalGameLogPages, p + 1)
                                )
                              }
