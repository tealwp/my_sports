...
export const LeagueDashboard: React.FC<Props> = ({ adapter }) => {
  ...
  if (loading) return <div className="p-6">Loading {adapter.leagueName} data...</div>;
  if (error) return <div className="p-6 text-red-600 font-bold">Error: {error}</div>;

  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">{adapter.leagueName} Standings</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {teams.map((team) => (
          <div key={team.id} className="border rounded-lg p-4 shadow hover:shadow-lg transition">
